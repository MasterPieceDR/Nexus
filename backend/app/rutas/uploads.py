...
def create_upload_url(payload: PresignedUploadRequest, user: User = Depends(get_current_user)):
    try:
        return create_presigned_upload_url(user.id, payload.filename, payload.content_type)
