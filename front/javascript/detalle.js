document.addEventListener("DOMContentLoaded", async () => {
  ...
  const loadComments = async () => {
    const comments = await PinCloudAPI.get(`/pins/${postId}/comments`);
    renderComments(comments);
  };

  try {
    const post = await PinCloudAPI.get(`/pins/${postId}`);
    detailCategory.textContent = post.CategoryName || "Sin categoría";
    detailTitle.textContent = post.Title;
    detailDescription.textContent = post.Description || "Sin descripción";
    detailAuthor.textContent = `Publicado por ${post.AuthorName}`;
    detailDate.textContent = `Fecha: ${new Date(post.PublishedAt).toLocaleDateString("es-EC")}`;
    detailStatus.textContent = `Estado: ${post.State}`;

    const isVideo = post.MediaKind === "VIDEO";
    const media = document.createElement(isVideo ? "video" : "img");
    media.src = post.MediaUrl || "../assets/placeholder.svg";
